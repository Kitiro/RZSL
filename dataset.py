...
class DataSet(Dataset):
    def __init__(
        ...
        if 0:
            ...
            # threshold_source, threshold_aux = 0.9, 0.5
            # mat_visual = sio.loadmat(os.path.join(self.data_dir, "res101.mat"))
            # features, labels = (
            #     mat_visual["features"].T,  # given feature
        ...
        if is_aux_feat:
            ...
            proc_mat_path = os.path.join('web_feat', f'proc_res101_{name}_features.mat')
            pre_mat_path = os.path.join('web_feat', f'pre_res101_{name}_features.mat')
            if not os.path.exists(proc_mat_path):
                print('start to filter noise aux features.')
                aux_mat_visual = sio.loadmat(pre_mat_path)
                aux_features, aux_labels = (
                    # aux_mat_visual["features"], # (36313, 2048)
                    P.scale(aux_mat_visual["features"], axis=0),
                    aux_mat_visual["labels"].astype(int).squeeze(), # (36313, )
                )
                # tr_cls_centroid_aux = np.zeros((np.unique(labels).shape[0], self.train_feature.shape[1]))  # 各个seen类的特征中心
                # for i in self.train_label_id:
                #     tr_cls_centroid_aux[i] = np.mean(aux_features[aux_labels == i], axis=0)
                
                # 通过G.T.的特征，来对aux feature进行筛选
                print('start to train classifiers for filtering aux features')
                from sklearn.neighbors import KNeighborsClassifier
                clfs = {i:None for i in self.train_label_id}

                for i in self.train_label_id:
                    clf = KNeighborsClassifier(n_neighbors=10)
                    true_idx = np.where(self.train_label==i)[0]
                    false_size = len(true_idx)*2
                    false_idx = np.random.choice(a=list(np.where(self.train_label!=i))[0], size=false_size, replace=False)
                    train_x = np.vstack((self.train_feature[true_idx], self.train_feature[false_idx]))
                ...
                sio.savemat(os.path.join(proc_mat_path), {
                    'features':aux_features_proc,
                    'labels': aux_labels_proc.astype(int)
                })
                aux_features, aux_labels = aux_features_proc, aux_labels_proc
            else:
                print(f'load processed aux features from {proc_mat_path}.')
                aux_mat_visual = sio.loadmat(proc_mat_path)
                aux_features, aux_labels = (
                    # aux_mat_visual["features"], # (36313, 2048)
            ...
            self.train_feature = np.vstack((self.train_feature, aux_features))
            self.train_label = np.hstack((self.train_label, aux_labels)).astype(int)
